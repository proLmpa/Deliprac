...
class ProductServiceTest {
    ...
    fun `update - happy path returns updated ProductInfo`() {
        ...
        val result = productService.update(storeId, productId, makeUpdateRequest(), ownerPrincipal)
    ...
    fun `update - store not found throws IllegalArgumentException`() {
        ...
        assertThatThrownBy { productService.update(storeId, productId, makeUpdateRequest(), ownerPrincipal) }
            .isInstanceOf(IllegalArgumentException::class.java)
            .hasMessage("Store not found")
    }
    ...
    fun `deactivate - happy path sets status to false`() {
        ...
        productService.deactivate(storeId, productId, ownerPrincipal)
    ...
    fun `deactivate - wrong owner throws IllegalStateException`() {
        ...
        assertThatThrownBy { productService.deactivate(storeId, productId, ownerPrincipal) }
            .isInstanceOf(IllegalStateException::class.java)
            .hasMessage("Forbidden")
    }
...
class ProductStatisticsServiceTest {
    ...
    fun `getPopularProducts - happy path returns sorted list`() {
        given(storeRepository.findById(storeId)).willReturn(Optional.of(makeStore()))
        given(productRepository.findTopByStoreIdOrderByPopularityDesc(storeId)).willReturn(listOf(makeProduct()))
